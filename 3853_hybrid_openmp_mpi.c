...
//Name : Batuwatta M.R.
//Reg. No. : EG/2020/353
//Project Title : European Option Pricing Using Monte Carlo Simulation (Hybrid MPI+OpenMP)
...
int main(int argc, char* argv[]) {
    ...
    double local_option_prices[NO_OF_COMPANIES];

    for(int i=0;i<NO_OF_COMPANIES;i++) {
        option_prices[i] = 0.0;
        local_option_prices[i] = 0.0;
    }
    int num_threads=2;
    ...
    for (int i=rank; i<NO_OF_COMPANIES;i+=numtasks) {
        Result result =CalculateSigmaAndS0(CSV_FILE[i],i);
        
        double TotalPayoff=0.0;
        
        #pragma omp parallel for reduction(+:TotalPayoff) schedule(static)
        ...
        local_option_prices[i]=OptionPrice;
    }
    MPI_Allreduce(local_option_prices, option_prices, NO_OF_COMPANIES, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
